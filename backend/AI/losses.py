...
def regress_onset_offset_frame_velocity_bce(model, output_dict, target_dict):
    ...
    onset_loss = bce(
        ...
        target_dict["reg_onset_roll"],
        target_dict["mask_roll"],
    )
    offset_loss = bce(
        output_dict["reg_offset_output"],
        target_dict["reg_offset_roll"],
        target_dict["mask_roll"],
    )
    frame_loss = bce(
        output_dict["frame_output"], target_dict["frame_roll"], target_dict["mask_roll"]
    )
    velocity_loss = bce(
...
def regress_pedal_bce(model, output_dict, target_dict):
    ...
    """
    onset_pedal_loss = F.binary_cross_entropy(
        output_dict["reg_pedal_onset_output"],
        target_dict["reg_pedal_onset_roll"][:, :, None],
    )
    offset_pedal_loss = F.binary_cross_entropy(
        output_dict["reg_pedal_offset_output"],
        target_dict["reg_pedal_offset_roll"][:, :, None],
    )
    frame_pedal_loss = F.binary_cross_entropy(
        output_dict["pedal_frame_output"], target_dict["pedal_frame_roll"][:, :, None]
    )
...
def google_pedal_bce(model, output_dict, target_dict):
    ...
    onset_pedal_loss = F.binary_cross_entropy(
        output_dict["reg_pedal_onset_output"],
        target_dict["pedal_onset_roll"][:, :, None],
    )
    offset_pedal_loss = F.binary_cross_entropy(
        output_dict["reg_pedal_offset_output"],
        target_dict["pedal_offset_roll"][:, :, None],
    )
    frame_pedal_loss = F.binary_cross_entropy(
        output_dict["pedal_frame_output"], target_dict["pedal_frame_roll"][:, :, None]
    )
