...
class TargetProcessor(object):
    ...
    def process(
        self, start_time, midi_events_time, midi_events, extend_pedal=True, note_shift=0
    ):
        ...
        pedal_dict = (
            {}
        )  # Used to store onset of pedal to be paired with offset of pedal
        ...
        for i in range(ex_bgn_idx, fin_idx):
            ...
            # Pedal
                ...
                attribute_list[0] == "control_change"
                and attribute_list[2] == "control=64"
            ):
        ...
        for note_event in note_events:
            ...
            piano_note = np.clip(
                note_event["midi_note"] - self.begin_note + note_shift,
                0,
                self.max_piano_note,
            )
            ...
            if 0 <= piano_note <= self.max_piano_note:
                bgn_frame = int(
                    round(
                        (note_event["onset_time"] - start_time) * self.frames_per_second
                    )
                )
                fin_frame = int(
                    round(
                        (note_event["offset_time"] - start_time)
                ...
                if fin_frame >= 0:
                    ...
                    if bgn_frame >= 0:
                        ...
                        reg_onset_roll[bgn_frame, piano_note] = (
                            note_event["onset_time"] - start_time
        ...
        for midi_note in buffer_dict.keys():
            piano_note = np.clip(
                midi_note - self.begin_note + note_shift, 0, self.max_piano_note
            )
            if 0 <= piano_note <= self.max_piano_note:
                bgn_frame = int(
                    round(
                        (buffer_dict[midi_note]["onset_time"] - start_time)
                        * self.frames_per_second
                    )
                )
        ...
        for pedal_event in pedal_events:
            bgn_frame = int(
                round((pedal_event["onset_time"] - start_time) * self.frames_per_second)
            )
            fin_frame = int(
                round(
                    (pedal_event["offset_time"] - start_time) * self.frames_per_second
                )
            )
            ...
            if fin_frame >= 0:
                ...
                reg_pedal_offset_roll[fin_frame] = (
                    pedal_event["offset_time"] - start_time
                ) - (fin_frame / self.frames_per_second)

                if bgn_frame >= 0:
                    pedal_onset_roll[bgn_frame] = 1
                    reg_pedal_onset_roll[bgn_frame] = (
                        pedal_event["onset_time"] - start_time
                    ) - (bgn_frame / self.frames_per_second)
    ...
    def extend_pedal(self, note_events, pedal_events):
        ...
        while pedal_events:  # Go through all pedal events
            ...
            while note_events:
                ...
                if (
                    pedal_event["onset_time"]
                    < note_event["offset_time"]
                    < pedal_event["offset_time"]
                ):
...
def plot_waveform_midi_targets(data_dict, start_time, note_events):
    ...
    x = librosa.core.stft(
        y=data_dict["waveform"], n_fft=2048, hop_length=160, window="hann", center=True
    )
    x = np.abs(x) ** 2
    ...
    axs[2].matshow(
        data_dict["offset_roll"].T, origin="lower", aspect="auto", cmap="jet"
    )
    axs[3].matshow(
        data_dict["reg_onset_roll"].T, origin="lower", aspect="auto", cmap="jet"
    )
    axs[4].matshow(
        data_dict["reg_offset_roll"].T, origin="lower", aspect="auto", cmap="jet"
    )
    axs[5].matshow(data_dict["frame_roll"].T, origin="lower", aspect="auto", cmap="jet")
    axs[6].matshow(
        data_dict["velocity_roll"].T, origin="lower", aspect="auto", cmap="jet"
    )
    axs[7].matshow(data_dict["mask_roll"].T, origin="lower", aspect="auto", cmap="jet")
    axs[8].matshow(
        data_dict["reg_pedal_onset_roll"][:, None].T,
        origin="lower",
        aspect="auto",
        cmap="jet",
    )
    axs[9].matshow(
        data_dict["reg_pedal_offset_roll"][:, None].T,
        origin="lower",
        aspect="auto",
        cmap="jet",
    )
    axs[10].matshow(
        data_dict["pedal_frame_roll"][:, None].T,
        origin="lower",
        aspect="auto",
        cmap="jet",
    )
...
class OnsetsFramesPostProcessor(object):
    ...
    def output_dict_to_note_pedal_arrays(self, output_dict):
        ...
        est_on_off_note_vels = self.output_dict_to_detected_notes(
            output_dict, frame_threshold=self.frame_threshold
        )
    ...
    def sharp_output_dict(self, output_dict, onset_threshold, offset_threshold):
        ...
        if "reg_onset_output" in output_dict.keys():
            output_dict["onset_output"] = self.sharp_output(
                output_dict["reg_onset_output"], threshold=onset_threshold
            )

        if "reg_offset_output" in output_dict.keys():
            output_dict["offset_output"] = self.sharp_output(
                output_dict["reg_offset_output"], threshold=offset_threshold
            )
    ...
    def sharp_output(self, input, threshold=0.3):
        ...
        for piano_note in range(classes_num):
            ...
            for i in range(1, frames_num - 1):
                if (
                    input[i, piano_note] > threshold
                    and input[i, piano_note] > input[i - 1, piano_note]
                    and input[i, piano_note] > input[i + 1, piano_note]
                ):
                    loct = i
                else:
