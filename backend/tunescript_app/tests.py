...
def test_upload_audio_file(client):
    ...
    with open("path/to/test/audio/file.mp3", "rb") as audio_file:
        response = graphql_query(
            query, variables={"file": audio_file, "title": "Test Audio"}, client=client
        )
    content = response.json()
    assert "errors" not in content
...
def test_create_transcription(client):
    ...
    audio_file = AudioFile.objects.create(
        user=user, title="Test Audio", audio_file="path/to/audio/file.mp3"
    )
    query = """
        mutation {
    ...
    assert "errors" not in content
    assert (
        content["data"]["createTranscription"]["transcription"]["title"]
        == "Test Transcription"
    )
...
def test_update_transcription(client):
    ...
    transcription = Transcription.objects.create(
        audio_file=audio_file, user=user, title="Test Transcription", status="PENDING"
    )
    query = """
        mutation {
    ...
    assert "errors" not in content
    assert (
        content["data"]["updateTranscription"]["transcription"]["title"]
        == "Updated Title"
    )
...
def test_rate_transcription(client):
    ...
    transcription = Transcription.objects.create(
        audio_file=audio_file, user=user, title="Test Transcription", status="COMPLETED"
    )
    query = """
        mutation {
...
def test_bookmark_transcription(client):
    ...
    assert "errors" not in content
    assert (
        content["data"]["bookmarkTranscription"]["favorite"]["transcription"]["title"]
        == "Test Transcription"
    )
