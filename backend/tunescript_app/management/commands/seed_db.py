...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            self.stdout.write(
                self.style.SUCCESS("Created admin user with confirmed email")
            )
        else:
        ...
        if User.objects.exclude(username="admin").exists():
            self.stdout.write(
                "Database already contains data, skipping further seeding."
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Created {len(users)} users in total with confirmed emails"
            )
        )
        ...
        for user in random.sample(users, min(3, len(users))):
            Favorite.objects.get_or_create(user=user, transcription=transcription)
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Created transcription: {transcription.title}")
        )
