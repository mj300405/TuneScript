...
class Transcription(models.Model):
    ...
    def update_rating(self, new_rating):
        with transaction.atomic():
            ...
            self.avg_rating = (F("avg_rating") * F("num_ratings") + new_rating) / (
                F("num_ratings") + 1
            )
            self.save()
    ...
    def update_rating_on_change(self, old_rating, new_rating):
        with transaction.atomic():
            self.avg_rating = (
                F("avg_rating") * F("num_ratings") - old_rating + new_rating
            ) / F("num_ratings")
            self.save()
            self.refresh_from_db()
...
class Rating(models.Model):
    ...
    transcription = models.ForeignKey(
        Transcription, on_delete=models.CASCADE, related_name="rating_set"
    )
