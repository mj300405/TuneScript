...
class TestMutations:
    ...
    def test_share_transcription_mutation(self, graphql_client, user):
        ...
        assert (
            ...
            in response["data"]["shareTranscription"]["shareUrl"]
        )
    ...
    def test_update_profile_mutation(self, graphql_client, profile):
        ...
        """
        variables = {"bio": "New bio", "public": True}
        response = self.execute_mutation(graphql_client, mutation, variables)
        assert "errors" not in response
    ...
    def test_add_to_favorites_mutation(self, graphql_client, user):
        ...
        response = self.execute_mutation(graphql_client, mutation, variables)
        assert "errors" not in response
        assert response["data"]["addToFavorites"]["favorite"]["transcription"][
            "id"
        ] == to_global_id("TranscriptionType", transcription.id)
        assert (
            response["data"]["addToFavorites"]["favorite"]["user"]["username"]
            == user.username
        )
    ...
    def test_remove_from_favorites_mutation(self, graphql_client, user):
        ...
        """
        variables = {
            "transcriptionId": to_global_id(
                "TranscriptionType", favorite.transcription.id
            )
        }
        response = self.execute_mutation(graphql_client, mutation, variables)
        assert "errors" not in response
        ...
        assert not Favorite.objects.filter(
            user=user, transcription=favorite.transcription
        ).exists()